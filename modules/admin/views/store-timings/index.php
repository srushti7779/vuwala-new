...
/* @var $searchModel app\modules\admin\models\search\StoreTimingsSearch */
...
$this->title = Yii::t('app', 'Store Timings');
...
<p>
    ...
    <?php if ($role == User::ROLE_ADMIN || $role == User::ROLE_SUBADMIN): ?>
        <?= Html::a(Yii::t('app', 'Create Store Timings'), ['create'], ['class' => 'btn btn-success']) ?>
    <?php elseif ($role == User::ROLE_VENDOR): ?>
        <?= Html::a(Yii::t('app', 'Create Store Timings'), ['create-store'], ['class' => 'btn btn-success']) ?>
...
<?php 
    $gridColumn = [
        ...
        [
                ...
                'label' => Yii::t('app', 'Day'),
                'value' => function($model){                   
                    return $model->day->title;                   
                },
                'filterType' => GridView::FILTER_SELECT2,
                'filter' => \yii\helpers\ArrayHelper::map(\app\modules\admin\models\Days::find()->asArray()->all(), 'id', 'title'),
                'filterWidgetOptions' => [
                    'pluginOptions' => ['allowClear' => true],
                ],
                'filterInputOptions' => ['placeholder' => 'Days', 'id' => 'grid-store-timings-search-day_id']
            ],
    ...
    <?= GridView::widget([
        ...
        'columns' => $gridColumn,
        'pjax' => true,
        'pjaxSettings' => ['options' => ['id' => 'kv-pjax-container-store-timings']],
        'panel' => [
