...
/**
 * This is the base model class for table "shop_review".
 *
 * @property integer $id
...
class ShopReview extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['vendor_details_id', 'user_id', 'order_id'], 'required'],
            [['vendor_details_id', 'user_id', 'order_id', 'type_id', 'status', 'create_user_id', 'update_user_id'], 'integer'],
    ...
    /**
     * @inheritdoc
     * @return \app\modules\admin\models\ShopReviewQuery the active query used by this AR class.
     */
    public static function find()
    {
        return new \app\modules\admin\models\ShopReviewQuery(get_called_class());
    }
    public function asJson()
    {
        ...
        $data['user_full_name'] =  $this->user->first_name . ' ' . $this->user->last_name;

        $data['profile_image'] =  $this->user->profile_image;


        $data['order_id'] =  $this->order_id;
        if(!empty( $this->order->orderDetails)){
            foreach($this->order->orderDetails as $orderDetails){
            $data['order'][] = $orderDetails->service->asJsonByOrder();
            }
 ...
 $data['order']=[];
    ...
    public function asJsonReview($user_id = '', $latitude = '', $longitude = '')
    {
        ...
        $data['vendorDetails'] =  $this->vendorDetails->asJsonReview($user_id, $latitude, $longitude); 

        $data['user_id'] =  $this->user_id;

        $data['order_id'] =  $this->order->asJsonReview();  
        ...
        $data['user_rated_time'] =  $this->created_on;
