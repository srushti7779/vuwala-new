...
class AuthSession extends BaseAuthSession
{
    ...
    public static function newSession($model)
    {
        ...
        $auth_session->type_id        = $model->device_type ?? 1;
        $auth_session->created_on     = date('Y-m-d H:i:s');
        if ($auth_session->save()) {
            return $auth_session;
        } else {
            print_r($auth_session->getErrors());
            exit;
        }
    }
    ...
    {
        $model = AuthSession::find()->where(['create_user_id' => $id])->all();
        if (!empty($model)) {
            foreach($model as $modelData){
                $device_token_data[] = $modelData->device_token;
            }
            return $device_token_data;
        }
    }

    public function getDeviceTokenUser($id){
