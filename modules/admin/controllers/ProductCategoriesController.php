...
use app\modules\admin\models\search\ProductCategoriesSearch;
...
class ProductCategoriesController extends Controller
{
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['index', 'view', 'create', 'update', 'delete', 'update-status', 'add-sku'],
    ...
    public function actionView($id)
    {
        ...
        $providerSku = new \yii\data\ArrayDataProvider([
            'allModels' => $model->skus,
        ]);
    ...
    public function actionDelete($id)
    {
        ...
        $model = $this->findModel($id);
        if(!empty($model)){
            $model->status = ProductCategories::STATUS_DELETE;
            $model->save(false); 
        }
    ...
    public function actionUpdateStatus(){
		...
		if (! empty ( $post ['id'] ) ) {
			$model = ProductCategories::find()->where([
				'id' => $post['id'],
			])->one();
			if(!empty($model)){
    ...
    */
    public function actionAddSku()
    {
        if (Yii::$app->request->isAjax) {
            $row = Yii::$app->request->post('Sku');
            if (!empty($row)) {
                $row = array_values($row);
            }
            if((Yii::$app->request->post('isNewRecord') && Yii::$app->request->post('_action') == 'load' && empty($row)) || Yii::$app->request->post('_action') == 'add')
                $row[] = [];
            return $this->renderAjax('_formSku', ['row' => $row]);
        } else {
            throw new NotFoundHttpException(Yii::t('app', 'The requested page does not exist.'));
        }
    }
