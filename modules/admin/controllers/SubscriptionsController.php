...
use app\modules\admin\models\search\SubscriptionsSearch;
...
class SubscriptionsController extends Controller
{
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['index', 'view', 'create', 'update', 'delete', 'update-status', 'add-vendor-subscriptions'],
    ...
    public function actionView($id)
    {
        ...
        $providerVendorSubscriptions = new \yii\data\ArrayDataProvider([
            'allModels' => $model->vendorSubscriptions,
        ]);
        return $this->render('view', [
            'model' => $this->findModel($id),
            'providerVendorSubscriptions' => $providerVendorSubscriptions, 
        ]);
    }
    ...
    public function actionDelete($id)
    {
        ...
        $model = $this->findModel($id);
        if(!empty($model)){
            $model->status = Subscriptions::STATUS_DELETE;
            $model->save(false); 
        }
    ...
    public function actionUpdateStatus(){
		...
		if (! empty ( $post ['id'] ) ) {
			$model = Subscriptions::find()->where([
				'id' => $post['id'],
			])->one();
			if(!empty($model)){
    ...
    * for VendorSubscriptions
    * @author Yohanes Candrajaya <moo.tensai@gmail.com>
    * @author Jiwantoro Ndaru <jiwanndaru@gmail.com>
    *
    * @return mixed
    */
    public function actionAddVendorSubscriptions()
    {
        if (Yii::$app->request->isAjax) {
            $row = Yii::$app->request->post('VendorSubscriptions');
            if (!empty($row)) {
                $row = array_values($row);
            }
            if((Yii::$app->request->post('isNewRecord') && Yii::$app->request->post('_action') == 'load' && empty($row)) || Yii::$app->request->post('_action') == 'add')
                $row[] = [];
            return $this->renderAjax('_formVendorSubscriptions', ['row' => $row]);
        } else {
            throw new NotFoundHttpException(Yii::t('app', 'The requested page does not exist.'));
        }
    }
