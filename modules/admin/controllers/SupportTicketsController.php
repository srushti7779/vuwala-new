...
use app\modules\admin\models\search\SupportTicketsSearch;
...
class SupportTicketsController extends Controller
{
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['index', 'view', 'create', 'update', 'delete', 'update-status', 'add-support-tickets-has-files'],
    ...
    public function actionView($id)
    {
        ...
        $providerSupportTicketsHasFiles = new \yii\data\ArrayDataProvider([
            'allModels' => $model->supportTicketsHasFiles,
        ]);
        return $this->render('view', [
            'model' => $this->findModel($id),
            'providerSupportTicketsHasFiles' => $providerSupportTicketsHasFiles,
        ]);
    }
    ...
    public function actionDelete($id)
    {
        ...
        $model = $this->findModel($id);
        if(!empty($model)){
            $model->status = SupportTickets::STATUS_DELETE;
            $model->save(false); 
        }
    ...
    public function actionUpdateStatus(){
		...
		if (! empty ( $post ['id'] ) ) {
			$model = SupportTickets::find()->where([
				'id' => $post['id'],
			])->one();
			if(!empty($model)){
    ...
    * for SupportTicketsHasFiles
    * @author Yohanes Candrajaya <moo.tensai@gmail.com>
    * @author Jiwantoro Ndaru <jiwanndaru@gmail.com>
    *
    * @return mixed
    */
    public function actionAddSupportTicketsHasFiles()
    {
        if (Yii::$app->request->isAjax) {
            $row = Yii::$app->request->post('SupportTicketsHasFiles');
            if (!empty($row)) {
                $row = array_values($row);
            }
            if((Yii::$app->request->post('isNewRecord') && Yii::$app->request->post('_action') == 'load' && empty($row)) || Yii::$app->request->post('_action') == 'add')
                $row[] = [];
            return $this->renderAjax('_formSupportTicketsHasFiles', ['row' => $row]);
        } else {
            throw new NotFoundHttpException(Yii::t('app', 'The requested page does not exist.'));
        }
    }
